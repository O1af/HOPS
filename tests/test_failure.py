...
from hops.core.types import EventKind
...
def _make_failure_setup(device_fail_prob=0.0):
...
def test_recovery_clears_failed_state():
    ...
    np.random.seed(0)
    # Use long check interval so only one failure cycle occurs
    devices = [Device("gpu0", "gpu", 100, 8192, 1000)]
    topology = Topology(devices, [])
    collector = MetricsCollector()
    engine = EventEngine()
    config = {
        "check_interval": 100.0,  # long interval to avoid re-failure
        "device_fail_prob": 1.0,
        "link_fail_prob": 0.0,
        "recovery_time": 2.0,
    }
    fe = FailureEngine(engine, topology, collector, config)
